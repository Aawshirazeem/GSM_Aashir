<?php
	...
	class admin{
		...
		var $accessAdminMain = array(
                    ...
                    'Settings' => array('config_settings_2','smtp_settings','services_imei_countries','currency','config_gateway','services_imei_api','admin','web_maintinance','languages','translations'),
								...
								'cms' => array('config_reseller','invoice_edit','config_settings', 'config_news', 'manage_email', 'user_mail','slider','cms_pages','cms_menu','cms_settings'),
		...
		var $accessAdmin = array(
								...
								'dashboard' => array('dashboard', 'transections'),
																...
																'cms_pages'=>array('cms_pages'),

																'cms_menu'=>array('cms_menu'),

																'cms_settings'=>array('cms_settings'),
...

		{
			...
			$MemberLanguage = $cookie->getCookie($this->preCookie . 'MemberLanguage');
			if($MemberLanguage == ""){
				if(CONFIG_DEFAULT_LANG != ""){
					$this->setUserLang(CONFIG_DEFAULT_LANG);
					return $validator->getStr(CONFIG_DEFAULT_LANG);
					die;
				}
			}
			return $validator->getStr($MemberLanguage);
...

		{
			...
			$cookie->setCookie($this->preCookie . 'MemberLanguage',$value);

		}

		public function delUserLang()

		{

			$cookie = new cookie();

			$cookie->deleteCookie($this->preCookie . 'MemberLanguage',$value);
...

		{
...

			{
				...
				$sql= 'select am.id, am.username, am.password, am.user_lang, lm.file_name as language
...

			{
				...
				$sql= 'select am.id, am.username, am.password, two_step_auth, am.user_lang, lm.file_name as language
...

			{
...

             {
				...
				$this->setUserLang($row[0]["user_lang"]);
		...
		public function wordTrans($code,$word){

			$mysql = new mysql();

			$sql = 'select * from '.TRANSLATION_MASTER.' where lang_code = "'.$code.'" and ori_word = "'.$word.'"';
