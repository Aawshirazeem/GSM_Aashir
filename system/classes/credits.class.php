<?php
	class credits{
		...
		public function getMemberCredits()
		{
			...
			{
				$sql= 'select
								um.credits,um.ovd_c_limit, um.credits_inprocess, um.credits_used, um.currency_id,
								cm.prefix, cm.suffix, cm.rate
							from ' . USER_MASTER . ' um
							left join ' . CURRENCY_MASTER . ' cm on (um.currency_id = cm.id)
							where um.id=' . $member->getUserID();
				...
				$return['suffix'] = $suffix;
                                if($row['ovd_c_limit']!=0 && $row['ovd_c_limit']!="")
                                $return['ovd_c_limit'] =$this->printCredits($row['ovd_c_limit'], $prefix, $suffix);
                                else
                                $return['ovd_c_limit']="";
				$return['rate'] = $rate;
				return $return;
