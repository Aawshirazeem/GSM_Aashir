...
class api_eu_unlock{
	...
	public function api_get_code($args)
	{
		...
		$api_eu_unlock->sendCommand('IMEI_STATUS', array('id' => $args['extern_id']));
		
		$resultArray = $api_eu_unlock->parse2Array($api_eu_unlock->getResult());
		if(isset($resultArray['RESULT']['ERR']))
		{
	...
	public function api_place_order($args)
	{
		...
		$api_eu_unlock->sendCommand('IMEI_SUBMIT', array('imei' => $args['imei']));
		$resultArray = $api_eu_unlock->parse2Array($api_eu_unlock->getResult());
		if(isset($resultArray['RESULT']['ERR']))
		{
			//echo '<h2> Error Code: ' . $result['STATUS'] . '</h2>';
