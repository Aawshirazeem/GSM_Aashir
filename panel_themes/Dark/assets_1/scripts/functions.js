...
function toggleLayout() {
    ...
    if (layout === 'default-sidebar') {		
		if(jQuery.inArray("supplier", res) !== -1){
			 $('body').attr('data-layout', 'horizontal-navigation-1');
			config.layout = 'horizontal-navigation-1';
...
function toggleSection(id) {
	$('.list-unstyled.active').each(function(index, element) { 
		if($(this).find('.section-' + id).length > 0) return;
		var _id =  $(this).parent().find('.sideline').attr('data-id');
		if(_id != id) closeSection(_id);
    });
	setTimeout(function(){
...
function closeSection(id) {
		$('.section-' + id).removeClass('active');
    	$('.fa-caret-down.icon-' + id).removeClass('fa-rotate-180');
};
