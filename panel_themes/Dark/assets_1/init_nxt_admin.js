...
function apiSync(type, id,chk_services)
{
	...
	switch(type)
	{
			...
			$.ajax({
				...
				success: function(msg){
					...
					$('.progress-bar-sync').css("width", '25%');
                                         var str =msg;
                                         //var m=n=o=0;
                                        var n = str.indexOf("Could Not Resolve");
                                         var o = str.indexOf("Failed to connect");
                                        //var m = str.indexOf("Could Not Resolve");
                                        var m=0;
                                        m = str.indexOf("text-danger");
                                       //  alert(n);
                                        if(n==-1 && m==-1 && o==-1)
					apiSync(2, id,chk_services)
                                    else
                                    {
                                        $('#load_details').scrollTop($('#load_details')[0].scrollHeight);
		...
		case 2:
			...
			$.ajax({
				...
				data: 'rand=' + randNum + "&id=" + id + "&chk_services=" + chk_services,
				error: function(){
					alert("Can't complete process at present please try after some time!");
				},
				success: function(msg){
