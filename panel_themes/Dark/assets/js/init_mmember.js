...
//	alert(total);
	$("#valRate").html(prefix + "1" + suffix + " = " + defaultPrefix + ratePer.toFixed(2) + defaultSuffix);
	$("#valCr").html(prefix + " " + credits.toFixed(2) + defaultSuffix);
	$("#valPer").html(per + "%");
	$("#valCharges").html(prefix + " " + charges.toFixed(2) + defaultSuffix);
	$("#valToPay").html(prefix + " " + total.toFixed(2) + defaultSuffix);
    }
}
...
function imeiToolDetails()
{
	...
	$.ajax({
	   ...
	   success: function(msg){
	   	...
	   	$("#toolDetails").hide();
		  $("#toolDetails").removeClass("hidden");
		   $("#toolDetails").html(msg);
		   $("#toolDetails").fadeIn();
                 //  alert('aaaaaaa');
                    imeiToolDetails2();
...
function imeiToolDetails2()
{
     //setPathsMember('','/impkk/user/');here
    // alert('hehehe');
	//$('#loadIndTool').show();
       // $("#toolDetails").hide();
	//$("#toolDetails").html('<div style="text-align:center;width:100%;font-size:16px;line-height:16px;"><br /><br /><img src="/images/progress.gif" /> Please wait...</div>');
	...
	$.ajax({
	   type: "GET",
	   url: config_path_site_member + "_ajax_load_tool_details_2.do",
	   data: "temp=" + randNum + '&tool=' + $("#tool").val(),
	   error: function(){
		 $("#toolDetails").html('<div class="warning">Can\'t complete process at present please try after some time!</div>');
		 $("#loadIndTool").hide();
