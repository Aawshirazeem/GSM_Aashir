...
$sql_tools = 'select
							...
							left join ' . SERVER_LOG_AMOUNT_DETAILS . ' itad on(slm.id = itad.log_id and cm.id = itad.currency_id)';

$tools = $mysql->getResult($sql_tools);



$sql_details = 'select currency_id, log_id, amount from ' . PACKAGE_SERVER_LOG_DETAILS . ' where package_id=' . $package_id . ' order by log_id, currency_id';
...
<div class="row m-b-20">
	...
	<div class="col-xs-12">
    	...
    	<ol class="breadcrumb icon-home icon-angle-double-right animation-delay-slow">
            ...
            <li class="slideInDown wow animated active"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_server_log_list')); ?></li>
...
<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>package_server_logs_credit_process.do" method="post">
    ...
    <div class="row">
    	...
    	<div class="col-xs-10">
        	...
        	<h4 class="m-b-20"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_server_log_list')); ?></h4>
            ...
            <table class="table table-hover table-striped">
                ...
                if ($tools['COUNT'] > 0) {
                    ...
                    foreach ($tools['RESULT'] as $tool) {
                        ...
                        echo '<td>' . $mysql->prints($tool['server_log_name']) . '</td>';

                        echo '<td class="text_right">

                                        ' . $tool['prefix'] . $tool['amount'] . '

                                        <input type="hidden" name="org_' . $tool['id'] . '" value="' . $tool['amount'] . '" />
