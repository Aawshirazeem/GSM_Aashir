...
if ($package_id == "") {
    header("location:" . CONFIG_PATH_SITE_ADMIN . "imei_package_special_credit_add.html?reply=" . urlencode('reply_credit_mising'));
    exit();
}


$sql_tools = 'select id from ' . IMEI_TOOL_MASTER;
...
$check = 0;
$sql_1 = 'delete from  ' . PACKAGE_IMEI_DETAILS . ' where package_id=' . $package_id;
$mysql->query($sql_1);
...
foreach ($tools['RESULT'] as $tool) {
    foreach ($currencies['RESULT'] as $currency) {
        ...
        if ($request->PostStr('amount_' . $tool['id'] . '_' . $currency['id']) != '') {
            $sql.='(
						' . $package_id . ',
						' . $currency['id'] . ', 
						' . $tool['id'] . ', 
