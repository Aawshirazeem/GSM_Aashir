...
?>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>report_order_summary_server_log.html" method="get" name="frm_customer_edit_login" id="frm_customer_edit_login" class="formSkin">
						<fieldset>
						<legend></legend>
						<div class="form-group">
							...
							<p class="btn-group" style="text-align:center">
	...
	<section class="panel MT10">
		<div class="panel-heading">
			<?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_server_log_report')); ?>
			<div class="btn-group MB10 pull-right">
				...
				<a href="report_order_summary_server_log.html" class="btn btn-primary btn-xs"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_server_log')); ?> </a>
		...
		<?php
			$i=0;
			$qType = ' where year(oslm.date_time)=date_format(now(),"%Y") ';

			if($from_date != '' && $to_date != '')
			{
				$from_date_search=date('Ymd',strtotime($from_date));
				$to_date_search=date('Ymd',strtotime($to_date));
				$qType = ' where date(oslm.date_time) between ' . $from_date_search . ' and ' . $to_date_search;
			}
			...
			if($mysql->rowCount($query_1))
			{
				...
				foreach($rows_1 as $row_1)
				{
					...
					if($group_name != $row_1['group_name'])
						{
							echo '<tr><td></td><td colspan="5">' . $row_1['group_name'] . '</td></tr>';
