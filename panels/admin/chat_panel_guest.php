...
<div class="col-lg-4">
    ...
    <table id="data5" class=" table table-rep-plugin">
        <tr>
            <th></th>
            <th></th>
            <th colspan="2"><input type="text" id="search2" placeholder="<?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_Search_Contact')); ?>" class="form-control"></input></th>
...
// $sql = 'select * from ' . Vendor . ' order by id';
        ...
        $sql = "select if(a.uname='','Guest',a.uname) username,a.uemail,a.user_id,1 as online from " . Chat_Box_NEW . " a  where a.admin_id=" . $admin->getUserId() . " group by a.user_id";
        ...
        if ($result['COUNT']) {
            foreach ($result['RESULT'] as $row) {
                ...
                $usernamenademail=$row['username'].' - '.$row['uemail'];
                ...
                if ($row['online'] == 1 && $latest <= 15) {
                    ...
                    echo '<td class="msgRow" onclick="delchat(' . $row['user_id'] . ',\'' . $usernamenademail . '\',this);">' . trim($mysql->prints($row['username'])) . '</td>';
                } else {
...
<div class="col-lg-8">
    ...
    <div class="col-xs-12" id="chat_panel_data"></div>
</div>
...
//    //alert(sendid+sendname);
    ...
    function getMSG()
    {
        ...
        $.ajax({
            type: "POST",
            url: config_path_site_admin + "chat_get_guest.do",
    ...
    function addMSG()
    {
        ...
        if (chatText == "") {
            ...
            $.ajax({
                type: "POST",
                url: config_path_site_admin + "chat_add_guest.do",
                ...
                success: function (msg) {
                    if (msg == 'notadd')
                    {
                        alert('User Offline');
                        delchatguest();
                         window.location.href = '<?php echo CONFIG_PATH_SITE_ADMIN; ?>chat_panel_guest.html';
                    }
...
//alert(e);
        ...
        $.ajax({
            type: "POST",
            url: config_path_site_admin + "_chat_guest.do",
    ...
    function delchatguest() {
      ...
      //  var userid = '<?php echo $userid; ?>';
       // var adminid = '<?php echo $adminid; ?>';
        ...
        $.ajax({
            type: "POST",
            url: config_path_site_admin + "_chat_del_guest.do",
