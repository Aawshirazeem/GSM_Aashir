<?php
	...
	$validator->formSetAdmin('services_imei_network_edit_534dfrr2');
	
	$id = $request->GetInt('id');
	$country_id = $request->GetInt('country_id');
	
	$sql ='select * from ' . IMEI_NETWORK_MASTER . ' where id=' . $mysql->getInt($id);
	$query = $mysql->query($sql);
	$rowCount = $mysql->rowCount($query);
	if($rowCount == 0)
	{
		header("location:" . CONFIG_PATH_SITE_ADMIN . "services_imei_networks.html?reply=" . urlencode('reply_success_id_missing'));
		exit();
	}
...
<div class="row">
	<div class="col-lg-12">
		<ul class="breadcrumb">
			...
			<li class="active"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_update_Network')); ?></li>
		</ul>
	</div>
</div>
<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>services_imei_networks_edit_process.do" method="post">
	<div class="row">
		<div class="col-md-6 col-md-offset-3">
			<div class="panel">
				<div class="panel-heading"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_update_Network')); ?></div>
				<div class="panel-body">
					<div class="form-group">
						<label><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_network_name')); ?> </label>
						<input name="network" type="text" class="form-control" id="network" value="<?php echo $row['network']?>" />
				...
				<div class="form-group">
					...
					<input type="submit" class="btn btn-success btn-sm" value="<?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_update_Network')); ?>" />
				</div>
				</div> <!-- / panel-body -->
 <!-- / panel-footer -->
			</div> <!-- / panel -->
