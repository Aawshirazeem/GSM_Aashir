...
if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $sql = "select a.id,a.credits,a.date_time from nxt_order_imei_master a
...
where a.user_id=$user_id LIMIT $page_position, $item_per_page";
     $sql = 'select im.*, im.id as imeiID,
								...
								left join ' . CURRENCY_MASTER . ' cm on(cm.id = ' . $user_id. ')
								...
								where im.user_id=' . $user_id . ' order by im.id DESC 
...
<?php
    ...
    foreach ($rows as $row)
    {
	 ...
	 $order_reply= 'Page Not found';
                                                ...
                                                ?>
                            <td><small><?php echo nl2br($order_reply); ?> </small></td>
                                <?php
                                              }
                                            else 
                                              {
                            ...
                            <td><small>---</small></td>
                                <?php 
                                        ...
                                        switch ($row['status']) {
                                            case 0:
                                                echo '<span class="label label-default">New_Order</span>';
                                                break;
                                            case 1:
                                                echo '<span class="label label-primary">In_Process</span>';
                                                break;
                                            case 2:
