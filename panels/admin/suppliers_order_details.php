...
$sql = 'select * from ' . SUPPLIER_MASTER . ' where id=' . $user_id;
...
$sqlCount = 'select im.status, count(im.id) as total
					...
					(im.tool_id in (select tool from ' . IMEI_SUPPLIER_DETAILS . ' where supplier_id=' . $user_id . ')
...
<div class="row m-b-20">
	...
	<div class="col-xs-12">
    	...
    	<ol class="breadcrumb icon-home icon-angle-double-right animation-delay-slow">
            ...
            <li class="slideInDown wow animated"><a href="<?php echo CONFIG_PATH_SITE_ADMIN; ?>suppliers.html"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_manage_suppliers')); ?></a></li>
...
$sql_tool = 'select 
										...
										where  (itm.id in (select tool from ' . IMEI_SUPPLIER_DETAILS . ' where supplier_id=' . $user_id . ')
...
$sql_tool = 'select
										...
										left join ' . IMEI_SUPPLIER_DETAILS . ' sd on (tm.id = sd.tool and sd.supplier_id = ' . $user_id . ')
...
?>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <label class="c-input c-checkbox"><input type="checkbox" name="imei_prefix" /> <span class="c-indicator c-indicator-danger"></span><?php echo $admin->wordTrans($admin->getUserLang(),'Remove everything before IMEI number'); ?></label>
...
?>
        ...
        <h4 class="m-b-10">
            ...
            <div class="btn-group pull-right">
                ...
                <div class="btn-group extra">
                    ...
                    <a href="<?php echo CONFIG_PATH_SITE_ADMIN; ?>suppliers_order_details.html?type=avail<?php echo (($pString != '') ? ('&' . $pString ) : ''); ?>" class="btn <?php echo ($type == 'avail') ? 'btn-primary btn-sm' : 'btn-default btn-sm'; ?>"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_available')); ?> </a>

                    <a href="<?php echo CONFIG_PATH_SITE_ADMIN; ?>suppliers_order_details.html?type=rejected<?php echo (($pString != '') ? ('&' . $pString ) : ''); ?>" class="btn <?php echo ($type == 'rejected') ? 'btn-primary btn-sm' : 'btn-default btn-sm'; ?>"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_rejected')); ?> </a>

                     

                    <a href="<?php echo CONFIG_PATH_SITE_ADMIN; ?>suppliers_order_details.html<?php echo ($pString != '') ? ('?' . $pString) : '' ?>" class="btn <?php echo ($type == '') ? 'btn-primary btn-sm' : 'btn-default btn-sm'; ?>"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_all_orders')); ?> </a>
            ...
            <div class="MT10">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tr>
                        ...
                        <th width="16"><label class="c-input c-checkbox"><input type="checkbox" class="autoCheckAll"  id="kaka" data-class="checkAllIMEIS" /><span class="c-indicator c-indicator-success"></span></label></th>
                        ...
                        if ($type != 'pending') {
                            ...
                            echo '<th>
											...
											<a href="#" class="toggle" id="code">' . $lang->get('lbl_unlock_code') . ' <i class="fa fa-info-sign"></i></a><br />
                            ...
                            echo '
									...
									<th width="100">
										...
										<a href="#" class="toggle" id="unText">
											...
											'. $admin->wordTrans($admin->getUserLang(),'Unavail').' <i class="fa fa-info-sign"></i></a>
                    ...
                    switch ($type) {
                        ...
                        case 'locked':
                            ...
                            $qType .= ' and im.supplier_id=' . $user_id;
                        ...
                        default:
                            ...
                            $qType = (($qType != '') ? ' and ' : '') . ' im.supplier_id=' . $user_id;
                    ...
                    $sql = 'select im.*, im.id as imeiID,
									...
									where (im.tool_id in (select tool from ' . IMEI_SUPPLIER_DETAILS . ' where supplier_id=' . $user_id . ')
                    ...
                    if ($totalRows > 0) {
                        ...
                        foreach ($rows as $row) {
...

                            {
                                ...
                                echo '<label class="c-input c-checkbox"> <input type="checkbox" class="subSelectLock checkAllIMEIS" name="locked_' . $row['id'] . '" value="' . $row['id'] . '"><span class="c-indicator c-indicator-danger"></span>';
                            ...
                            if ($type != "pending") {
                                ...
                                if (defined("DEMO")) {
                                    ...
                                    echo '*****'.$admin->wordTrans($admin->getUserLang(),'Demo').'*****';
                                    ...
                                    if (($type == 'locked' and $row['status'] == '1') or $type == 'avail' or $type == 'verify') {
                                        ...
                                        echo ($row['reply'] != '') ? nl2br(base64_decode($row['reply'])) : $row['message'];
                        ...
                        echo '<tr><td colspan="7" class="no_record">' . $admin->wordTrans($admin->getUserLang(),$lang->get('lbl_no_record_found')) . '</td></tr>';
                    ...
                    if ($type == 'pending' || $type == 'locked' || $type == 'verify') { // || $type == 'avail' || $type == 'verify')
                        ...
                        echo '
							...
							<form action="' . CONFIG_PATH_SITE_SUPPLIER . 'order_imei_' . (($type != '') ? $type : 'all') . '_process_ajax.do" enctype="multipart/form-data" method="post" name="frmAjaxOrderSub" id="frmAjaxOrderExtra">
								...
								<input type="submit" name="process" class="btn btn-success btn-sm" value="' . $admin->wordTrans($admin->getUserLang(),$lang->get('lbl_process_imeis')) . '" />
    ...
    <div class="btn-group pull-left">
        ...
        <a href="<?php echo CONFIG_PATH_SITE_SUPPLIER; ?>order_imei.html?limit=100<?php echo (($pStringLimit) ? ('&' . $pStringLimit) : ''); ?>" class="btn btn-default btn-xs">100</a>

        <a href="<?php echo CONFIG_PATH_SITE_SUPPLIER; ?>order_imei.html?limit=200<?php echo (($pStringLimit) ? ('&' . $pStringLimit) : ''); ?>" class="btn btn-default btn-xs">200</a>

        <a href="<?php echo CONFIG_PATH_SITE_SUPPLIER; ?>order_imei.html?limit=500<?php echo (($pStringLimit) ? ('&' . $pStringLimit) : ''); ?>" class="btn btn-default btn-xs">500</a>

        <a href="<?php echo CONFIG_PATH_SITE_SUPPLIER; ?>order_imei.html?limit=1000<?php echo (($pStringLimit) ? ('&' . $pStringLimit) : ''); ?>" class="btn btn-default btn-xs">1000</a>
    ...
    <div class="TA_C navigation"><?php if ($imei == '') {
