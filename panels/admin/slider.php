...
<div class="row m-b-20">
	<div class="col-xs-12">
    	<ol class="breadcrumb icon-home icon-angle-double-right animation-delay-slow">
            ...
            <li class="slideInDown wow animated active"><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_Slider')); ?></li>
        </ol>
    </div>
</div>
...
<div class="m-t-10">
	<h4 class="m-b-20 clearfix">
		<?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_Manage_Slider')); ?>
        <a href="#" class="btn btn-danger btn-sm pull-right btnAddNewPage"> <i class="fa fa-plus"></i> <?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('com_add_slider')); ?></a>
    ...
    <table class="table table-hover table-striped">
    	<tr>
        	<th width="16"></th>
            <th width="16"></th>
            <th><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_Title')); ?></th>
            <th width=""><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_image')); ?></th>
            <th width=""><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_status')); ?></th>
            <th width=""><?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_display_order')); ?></th>
    ...
    <!-- Modal content-->
    	<div class="modal-content">
        	<div class="modal-header">
                ...
                <h4 class="modal-title"><?php echo $admin->wordTrans($admin->getUserLang(),'Slider'); ?></h4>
           	</div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label> <?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_title')); ?> </label>
                                <input name="sliderTitle" type="text" class="form-control sliderTitle" id="sliderTitle" value="" autocomplete="off" />
                            </div>
                            <div class="form-group">
                                <label> <?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_display_order')); ?> </label>
                                <input name="sliderDispOrder" type="number" class="form-control sliderDispOrder" id="sliderDispOrder" value="" autocomplete="off" />
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                            	<label> <?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_notes')); ?> </label>
...
$(document).ready(function(e) {
	...
	$('.btnBeforeClick').click(function(e){
		e.preventDefault();
		if($('#sliderImage').val() != "" && $('#sliderTitle').val() != "" && $('#sliderDispOrder').val() != ""){
			...
			if($('.imgResp').is(':empty')){
			  alert('Slider Title,Display Order and Image is Required');
			}else{
				var _form = $(this).closest('form');
				var textareaValue = $('#summernote').summernote('code');
				$('.notes').val(textareaValue);
	...
	$(document).on('click','.btnEditSlider',function(e){
		...
		$.ajax({
			...
			if(resp.status == 1){
				...
				$('.sliderTitle').val(resp.title);
				$('.sliderDispOrder').val(resp.dispOrder);
