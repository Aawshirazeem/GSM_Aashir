<?php
	...
	$validator->formSetAdmin('services_imei_mep_group_edit_4565646');

	$id = $request->GetInt('id');
	
	$sql ='select * from ' . IMEI_MEP_GROUP_MASTER . ' where id=' . $mysql->getInt($id);
	$query = $mysql->query($sql);
	$rowCount = $mysql->rowCount($query);
	if($rowCount == 0)
	{
		header("location:" . CONFIG_PATH_SITE_ADMIN . "services_imei_mep_groups.html?reply=" . urlencode('reply_service_imei_mep_groups'));
		exit();
	}
...
<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>services_imei_mep_groups_edit_process.do" method="post">
			...
			<div class="panel">
				...
				<div class="panel-body">
					<div class="form-group">
						...
						<input name="mep_group" type="text" class="form-control" id="mep_group" value="<?php echo $row['mep_group']?>" />
				...
				<div class="form-group">
					...
					<input type="submit" value="<?php echo $admin->wordTrans($admin->getUserLang(),$lang->prints('lbl_update_mep_group')); ?>" class="btn btn-success btn-sm" />
				</div>
				</div> <!-- / panel-body -->
 <!-- / panel-footer -->
			</div> <!-- / panel -->
