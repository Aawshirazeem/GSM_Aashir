...
if(isset($_POST['isEdit'])){
	...
	$resp = array('status'=>1,'id'=>$nId,'title'=>$title,'image'=>$image,'notes'=>$notes);
...
if(isset($_FILES['sliderImage']['name']) && $_FILES['sliderImage']['name'] != ""){
	...
	$sql = 'update ' . SLIDER_MASTER . ' set slider_title = ' . $mysql->quote($title) . ', image = '.$mysql->quote($randName).', notes = '.$mysql->quote(addslashes($notes)).', s_width = '.$sliderWidth .', s_height = '.$sliderHeight .' where id = ' . $id;
	$mysql->query($sql);
	header("location:" . CONFIG_PATH_SITE_ADMIN . "slider.html?msg=" . urlencode("Slider Updated Successfully"));
	
}else{
	$sql = 'update ' . SLIDER_MASTER . ' set slider_title = ' . $mysql->quote($title) . ', notes = '.$mysql->quote(addslashes($notes)).', s_width = '.$sliderWidth .', s_height = '.$sliderHeight .' where id = ' . $id;
	$mysql->query($sql);
	header("location:" . CONFIG_PATH_SITE_ADMIN . "slider.html?msg=" . urlencode("Slider Updated Successfully"));
}
?>