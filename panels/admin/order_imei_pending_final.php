<?php
	...
	if(!isset($_POST['Ids']))
	{
		header("location:" .CONFIG_PATH_SITE_ADMIN ."order_imei.html?type=pending&reply=" .urlencode('reply_no_imei_seleted'));
		exit();
	}
	$ids = $_POST['Ids'];
	...
	$newIds = trim($newIds, ',');
	$sql = 'select
						oim.id, oim.imei, itm.tool_name,oim.custom_value,oim.custom_1,oim.custom_2,oim.custom_3,oim.custom_4,oim.custom_5
	...
	foreach($rows as $row)
	{
		...
		$imeis .= '<span class="tplId">' . $row['id'] . ': </span>' . trim($row['imei']).''.$row['custom_1'].''.$row['custom_2'].''.$row['custom_3'].''.$row['custom_4'].''.$row['custom_5']."\n"; //. " ===>" .$row['custom_value'] ."\n";
...
?>
	...
	<div class="WD90 hidden" id="imeiPlain" rows="15" style="font-family:monospace;"><?php echo nl2br($imeis);?></div>
	<div class="WD90 hidden" id="imeiPlain2" rows="15" style="font-family:monospace;"><?php echo nl2br($imeis);?></div>

	

<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>order_imei_pending_final_download.do" method="post" name="frm_customer_edit_login" id="frm_customer_edit_login">
...
<form action="<?php echo CONFIG_PATH_SITE_ADMIN; ?>order_imei_pending_final_email.do" target="_blank" class="form-inline" method="post">
