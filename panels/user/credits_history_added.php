	...
	<table class="table table-striped table-hover">
		...
		<?php
			...
			$sql = 'select ctm.* , um.username as username1, um2.username as username2
					...
					where (user_id=' . $member->getUserID() . ' or user_id2=' . $member->getUserID() . ') and ctm.info="Credits Added by Admin"
					order by ctm.id DESC';
			$result = $mysql->getResult($sql, false, 20, $offset, CONFIG_PATH_SITE_USER . 'credits_history_added.html', array());
			//echo $sql;
                       // var_dump($result);
                       // exit;
                        $strReturn = "";
