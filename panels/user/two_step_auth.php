...
if ($type == 0 && $code == "") {
    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_failed_code_empty'));
    exit();
} else if ($type == "") {
    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_failed'));
    exit();
} else {
...
if ($type == 0) {
    ...
    if ($mysql->rowCount($query) > 0) {
        ...
        if ($mysql->rowCount($query) > 0) {
            ...
            if ($googlekey != "") {
                ...
                if ($checkResult) {
                    $sqladd = 'update ' . USER_MASTER . ' set google_auth_key=' . $mysql->quote($googlekey) . ', two_step_auth=1 where id=' . $member->getUserId();
                    $mysql->query($sqladd);
                    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_success_on'));
                    exit();
                    // echo 'OK';
                } else {
                    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_failed'));
    ...
    // echo 'disable it';
    $sqladd = 'update ' . USER_MASTER . ' set two_step_auth=0 where id=' . $member->getUserId();
    $mysql->query($sqladd);
    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_success_off'));
    exit();
    exit;
} else {
    header("location:" . CONFIG_PATH_SITE_USER . "account_details.html?reply=" . urlencode('reply_failed'));
