<?php
	...
	foreach($Ids as $id)
	{
		$sql = 'select um.imei,u.email,um.reply,um.credits,um.user_id
				from 
		...
		else
		{
			...
			if(($unlock_code != $unlock_code2) or (isset($_POST['verify_' . $id])))
			{
				...
				<p><b>IMEI:</b>' . $rows[0]['imei'] . '<p>
				<p><b>Unlock Code:</b>' .$mysql->quote(base64_decode($unlock_code)). '<p>
				<p><b>Credits:</b>' . $rows[0]['credits']. '<p>
				<p><b>Order ID :</b>' . $id. '<p>
				<p>--------------------------------------------------				
				';
	...
	header("location:" .CONFIG_PATH_SITE_SUPPLIER ."order_imei.html?type=" . $type . "&supplier_id=" . $supplier_id . "&reply=" .urlencode('reply_imei_update_success'));
	exit();
?>