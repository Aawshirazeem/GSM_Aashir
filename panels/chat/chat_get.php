...
$sql=" select a.a_id,a.admin_id,a.user_id,a.msg,cast(a.time_stamp as time) msgtime,a.isview ,a.time_stamp
 ,b.nname adminname, '' as img
 from nxt_chat_pool_new a
 inner join nxt_admin_master b
 on a.admin_id=b.id
 where a.admin_id=" . $admin_id . "
 and a.entry_type='user' 
 and a.isview=0 
...
if ($result['COUNT']) {
    ...
    foreach ($result['RESULT'] as $row) {
        ...
        $dtDateTime->setTimezone(new DateTimeZone($usertzz));
        $finaldate = $dtDateTime->format(' n/j/Y, h:i:s A ');
      //  $finaldate = $dtDateTime->format('l  \, F j\, Y h:i A');
        //$finaldate=$row['time_stamp'];
    ...
    $sql = 'update ' . Chat_Box_NEW . ' set isview=1
where a_id in (' . $msgid . ')';
    //echo $sql;
    $mysql->query($sql);
