...
<div class="container gsmColl">
    <h3 class="wow fadeInDown" data-wow-delay="0.2s">Prepaid Log Service</h3><br>
    <div class="clearfix">
...
//get defual currency
        ...
        $sqlgrp = 'select * from ' . PREPAID_LOG_GROUP_MASTER . ' a
...
order by a.sort_order';
        ...
        foreach ($rowsgrp as $grp) {
            ...
            $sql = 'select
										plm.*,
										plgm.group_name,
										if(' . $price . ' =1, plad.amount,"0") as amount,
										cm.prefix, cm.suffix
									...
									left join ' . PREPAID_LOG_AMOUNT_DETAILS . ' plad on(plad.log_id=plm.id and plad.currency_id = ' . $cur . ')
									where plgm.id=' . $temp_g_id . ' order by plgm.group_name, plm.prepaid_log_name';
    ...
    <tr class="commentlist">
        ...
        <th width="100">Credits</th>
                ...
                foreach ($rows as $pro) {
                    ...
                    echo '<td>' . $pro['id'] . '</td>';
                    echo '<td class="searchme"><b>' . $pro['prepaid_log_name'] . '</b></td>';
                    if ($price == 1)
